...
public class SyntaxTest extends TdfParserTest {
    ...
    public void test() {
        ...
        //[0] - or
        {
            ...
            assertEquals("or", production.getIdentifier().getIdentifier());
            ...
            assertEquals("non_terminal", production.getElements().get(0).asOr().getSecond().asNonTerminal().getIdentifier());
        }

        //[1] - non_terminal
        {
            final Production production = grammar.getProductions().get(1);
            assertEquals("non_terminal", production.getIdentifier().getIdentifier());
            assertEquals(1, production.getElements().size());
            assertEquals("terminal", production.getElements().get(0).asNonTerminal().getIdentifier());
        }

        //[2] - repeat
        {
            final Production production = grammar.getProductions().get(2);
            assertEquals("repeat", production.getIdentifier().getIdentifier());
            assertEquals(1, production.getElements().size());
            ...
            assertEquals("non_terminal", production.getElements().get(0).asRepeat().getElements()[1].asNonTerminal().getIdentifier());
        }

        //[3] - inline_action
        {
            final Production production = grammar.getProductions().get(3);
            assertEquals("inline_action", production.getIdentifier().getIdentifier());
            assertEquals(1, production.getElements().size());
        ...
        //[4] - optional
        {
            ...
            assertEquals("optional", production.getIdentifier().getIdentifier());
            assertEquals(1, production.getElements().size());
            assertEquals("A", production.getElements().get(0).asOptional().getElements()[0].asTerminalTag().getValue());
            assertEquals("non_terminal", production.getElements().get(0).asOptional().getElements()[1].asNonTerminal().getIdentifier());
        }

        //[5] - terminal
        {
            final Production production = grammar.getProductions().get(5);
            assertEquals("terminal", production.getIdentifier().getIdentifier());
            assertEquals(1, production.getElements().size());
        ...
        //[6] - repetition
        {
            ...
            assertEquals("repetition", production.getIdentifier().getIdentifier());
            assertEquals(1, production.getElements().size());
            ...
            assertEquals("non_terminal", production.getElements().get(0).asRepetition().getElement().asGroup().getElements()[1].asNonTerminal().getIdentifier());
        }

        //[7] - group
        {
            final Production production = grammar.getProductions().get(7);
            assertEquals("group", production.getIdentifier().getIdentifier());
            assertEquals(1, production.getElements().size());
            assertEquals("A", production.getElements().get(0).asGroup().getElements()[0].asTerminalTag().getValue());
            assertEquals("non_terminal", production.getElements().get(0).asGroup().getElements()[1].asNonTerminal().getIdentifier());
        }
    }
}
